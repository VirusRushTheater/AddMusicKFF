...
struct AddMusic_arglist
{
	...
	std::filesystem::path workDir {"."};		// Work folder where your song structure is stored.
...
class AddMusicK
{
	 ...
	 */
	AddMusicK(const AddMusic_arglist& addmusic_args, const std::filesystem::path& _workdir = ".") :
		p(addmusic_args),
		workdir(_workdir)
	{
