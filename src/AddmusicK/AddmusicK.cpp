...
void AddMusicK::tryToCleanAMMData()
{
	if ((rom.size() % 0x8000 != 0 && findRATS(0x078200)) || (rom.size() % 0x8000 == 0 && findRATS(0x078000)))		// Since RATS tags only need to be in banks 0x10+, a tag here signals AMM usage.
	{
		...
		execute( ((std::string)("perl addmusicMRemover.pl ")) + (std::string)ROMName, false);
		execute( ((std::string)("xkasAnti clean.smc ")) + (std::string)ROMName + "INIT.asm", false);
	}
}
...
void AddMusicK::cleanROM()
{
	...
	else
	{
		...
		{
			log.warning("WARNING: This ROM was modified using a newer version of AddmusicK. "
				"You can continue, but it is HIGHLY recommended that you upgrade AMK first." );
...
{
	unsigned int i;
	bool sputil_found = false;

	// CHANGES: Implemented this pattern searching as a regex in order to improve
	// efficiency and flexibility.
	std::string rom_as_string = (char *)rom.data();
	std::regex 	sputil_regex("New Super Mario World Sample Utility 2.0 by smkdan");
	std::smatch sputil_matches;

	// TODO: This regex.
	sputil_found = std::regex_search(rom_as_string, sputil_matches, sputil_regex);
	if (sputil_found)
		i = sputil_matches.position(0);
	...
	if (sputil_found == false)
	{
		log.debug("No Sample tool info detected.");
		return;
	}

	log.debug("Sample Tool detected. Erasing data...");
...
void AddMusicK::loadMusicList()
{
	...
	log.debug((std::stringstream{} << "Read in all " << shallowSongCount << " songs.").str());
...
void AddMusicK::assembleSPCDriver()
{
	...
	log.debug("Compiling main SPC program, pass 1.");
	...
	if (p.sfxDump && noSFX) {
		log.warning("The sound driver build does not support sound effects due to the !noSFX flag "
...
void AddMusicK::compileGlobalData()
{
	...
	log.info(std::stringstream("Total space used by all sound effects: 0x") << std::hex << std::setw(4) << std::uppercase << std::setfill('0') << (DF9DataTotal + DF9Count * 2 + DFCDataTotal + DFCCount * 2) << std::dec);
	...
	if (noSFX) {
		log.info("!noSFX is enabled in asm/UserDefines.asm, sound effects are not included");
		totalSizeStr = "Total size of main program: 0x";
	}
	else {
		totalSizeStr = "Total size of main program + all sound effects: 0x";
	}
	log.info(std::stringstream() << totalSizeStr << std::hex << std::uppercase << std::setw(4) << std::setfill('0') << programSize  << std::dec);

}

void AddMusicK::compileMusic()
{
	log.debug("Compiling music...");
...
{
	log.debug("Fixing song pointers...");
	...
	log.debug("Compiling main SPC program, final pass.");
	...
	log.debug(std::stringstream("Total space in ARAM left for local songs: 0x") << hex4 << (0x10000 - programSize - 0x400) << " bytes." << std::dec);
...
{
	if (p.checkEcho == false)		// If echo buffer checking is off, then the overflow may be due to too many samples.
	...
	*/
	...
	for (int mode = 0; mode <= maxMode; mode++)
	{
		...
		for (unsigned int i = 0; i < 256; i++)
		{
			...
			while (y > 0)
			{
				...
				log.debug(std::stringstream() << "Wrote \"" << fname << "\" to file.");
	...
	if (SPCsGenerated == 1)
		log.debug("Generated 1 SPC file.");
	else if (SPCsGenerated > 0)
		log.debug(std::stringstream() << "Generated " << SPCsGenerated << " SPC files.");

}

void AddMusicK::assembleSNESDriver2()
{
	log.debug("Generating SNES driver...");
	...
	log.debug("Writing sample files...");
